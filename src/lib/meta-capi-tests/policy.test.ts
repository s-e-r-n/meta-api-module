...
describe("policy", () => {
  it("makes a field required for every event", () => {
    expect(paths({ event_name: "ViewContent" })).toEqual(["custom_data.value"]);
    expect(
  ...
  it("makes a field required for one event", () => {
    expect(paths({ event_name: "Lead", custom_data: { value: 0 } })).toEqual([
      "user_data.em",
    ]);
    expect(
      browser_event_schema.safeParse({
  ...
  it("declares a custom event by naming it", () => {
    expect(
      browser_event_schema.safeParse({
        ...
        custom_data: { value: 0, promotion: "x" },
      }).success,
    ).toBe(true);
    expect(
      paths({ event_name: "ShareDiscount", custom_data: { value: 0 } }),
    ).toEqual(["custom_data.promotion"]);
    expect(
      paths({ event_name: "Undeclared", custom_data: { value: 0 } }),
    ).toEqual(["event_name"]);
  });
