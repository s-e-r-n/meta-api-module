...
beforeEach(() => {
  track.mockReset();
  track.mockResolvedValue({ ok: true, events_received: 1, fbtrace_id: "trace", messages: [], warnings: [] });
...
describe("MetaEvent", () => {
  ...
  it("fires once when it appears, with exactly the declared event", async () => {
    render(<MetaEvent event_name="ViewContent" custom_data={{ content_ids: ["42"], content_type: "product" }} />);
    await settle();
    expect(track).toHaveBeenCalledTimes(1);
    expect(track).toHaveBeenCalledWith({ event_name: "ViewContent", custom_data: { content_ids: ["42"], content_type: "product" } });
  });
  ...
  it("does not fire again on a rerender with the same declaration", async () => {
    const { rerender } = render(<MetaEvent event_name="ViewContent" custom_data={{ content_ids: ["42"] }} />);
    await settle();
    rerender(<MetaEvent event_name="ViewContent" custom_data={{ content_ids: ["42"] }} />);
    await settle();
    expect(track).toHaveBeenCalledTimes(1);
  });
  ...
  it("fires again when the declaration changes", async () => {
    const { rerender } = render(<MetaEvent event_name="ViewContent" custom_data={{ content_ids: ["42"] }} />);
    await settle();
    rerender(<MetaEvent event_name="ViewContent" custom_data={{ content_ids: ["43"] }} />);
    await settle();
    expect(track).toHaveBeenCalledTimes(2);
    expect(track).toHaveBeenLastCalledWith({ event_name: "ViewContent", custom_data: { content_ids: ["43"] } });
  });
