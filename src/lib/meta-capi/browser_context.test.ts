...
describe("browser_context", () => {
  it("captures the full page URL, query string included", () => {
    window.history.pushState({}, "", "/products?page=2&fbclid=Ab_C&utm_source=meta");
    expect(browser_context().event_source_url).toBe("http://localhost:3000/products?page=2&fbclid=Ab_C&utm_source=meta");
  });
