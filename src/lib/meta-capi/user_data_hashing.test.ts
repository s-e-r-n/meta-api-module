...
const hash_of = {
  john_smith: "62a14e44f765419d10fea99367361a727c12365e2520f32218d505ed9aa0f62f",
  ...
  swiss_phone: "641b2674c7b3d41f0c5167c163c845f3d899639ebc622d0d3b4632abeb14e6b0",
};
...
describe("hashed_user_data", () => {
  it("normalizes then hashes an email the way Meta's documentation shows", () => {
    expect(hashed_user_data({ em: "  John_Smith@gmail.com " }).user_data.em).toEqual([hash_of.john_smith]);
  });

  it("keeps digits only in a phone number and drops the leading zeros", () => {
    expect(hashed_user_data({ ph: "+1 (650) 555-1212" }).user_data.ph).toEqual([hash_of.us_phone]);
    expect(hashed_user_data({ ph: "0041 79 123 45 67" }).user_data.ph).toEqual([hash_of.swiss_phone]);
  });

  it("lowercases names, strips punctuation and keeps accents and other scripts", () => {
    const { user_data } = hashed_user_data({ fn: ["Mary", "Valéry", "정"], ln: "O'Brien" });
  ...
  it("takes a birth date as YYYYMMDD or YYYY-MM-DD and refuses anything ambiguous", () => {
    expect(hashed_user_data({ db: "1997-02-16" }).user_data.db).toEqual([hash_of.birth]);
    expect(hashed_user_data({ db: "19970216" }).user_data.db).toEqual([hash_of.birth]);
  ...
  it("reduces gender to its initial and drops anything else with a warning", () => {
    expect(hashed_user_data({ ge: "Female" }).user_data.ge).toEqual([sha256_hex("f")]);
  ...
  it("strips spaces from a postal code, keeps the part before a dash, and keeps five digits for the US", () => {
    expect(hashed_user_data({ zp: "M1 1AE" }).user_data.zp).toEqual([sha256_hex("m11ae")]);
    expect(hashed_user_data({ zp: "8000" }).user_data.zp).toEqual([sha256_hex("8000")]);
    expect(hashed_user_data({ zp: "94035-1234", country: "US" }).user_data.zp).toEqual([sha256_hex("94035")]);
    expect(hashed_user_data({ zp: "940351234", country: "US" }).user_data.zp).toEqual([sha256_hex("94035")]);
  });

  it("lowercases a country code and refuses anything but two letters", () => {
    expect(hashed_user_data({ country: "US" }).user_data.country).toEqual([hash_of.us]);
  ...
  it("trims and lowercases an external id before hashing, as Meta's SDK does", () => {
    expect(hashed_user_data({ external_id: " User-42 " }).user_data.external_id).toEqual([hash_of.user_42]);
  });

  it("passes an already hashed value through untouched", () => {
    expect(hashed_user_data({ em: hash_of.john_smith.toUpperCase() }).user_data.em).toEqual([hash_of.john_smith]);
  });
  ...
  it("refuses an IP address that is neither v4 nor v6", () => {
    ...
    expect(refused.warnings).toEqual([expect.stringContaining("client_ip_address")]);
    expect(hashed_user_data({ client_ip_address: "2001:db8::1" }).user_data.client_ip_address).toBe("2001:db8::1");
  });
});
