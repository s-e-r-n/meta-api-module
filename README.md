  ...
  return (
    <main>
      <MetaEvent
        ...
        custom_data={{ content_ids: [id], content_type: "product" }}
      />
      ...
    </main>
  );
...
For JSON that comes from outside your code, `send_inbound_meta_events(payload)` takes an unknown body shaped `{ "events": [ ... ] }` and returns the same `result`. The reference route `src/app/api/meta-events/route.ts` uses it: `POST /api/meta-events` with `Authorization: Bearer $META_CAPI_INBOUND_SECRET`.

## 5. What an event may carry

`event_name` is one of Meta's standard names, offered by autocompletion:
...
`Purchase` requires `custom_data.value` and `custom_data.currency`, and `AppendAttribution` requires `attribution_data` and `custom_data.currency`. The type checker refuses the declaration without them, and so does the server.

A custom event carries any name up to 50 characters, declared once with `custom_event` so that a misspelt standard name can never pass as a custom one:

```tsx
import { custom_event, MetaEvent } from "@/lib/meta-capi";

const share_discount = custom_event("ShareDiscount");

<MetaEvent event_name={share_discount} custom_data={{ promotion: "share_discount_10" }} />;
```
